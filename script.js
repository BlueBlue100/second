document.addEventListener('DOMContentLoaded', function() {
    ...
    manualForm.addEventListener('submit', function(event) {
        ...
        const asinInput = document.getElementById('urlInput').value.trim();
        if (asinInput) {
            // Add ASIN to the table
            addAsinToList(asinInput);
    ...
    bulkForm.addEventListener('submit', function(event) {
        ...
        reader.onload = function() {
            ...
            urls.forEach(url => {
                // Assuming each line in the file contains an ASIN
                const asin = url.trim();
                if (asin) {
                    addAsinToList(asin);
                }
    ...
    function addAsinToList(asin) {
        // Generate unique ID for the ASIN
        const id = Math.floor(Math.random() * 1000); // You can use a more robust method for generating IDs
    ...
    document.getElementById("manualForm").addEventListener("submit", function(event) {
        ...
        const asinInput = document.getElementById("urlInput").value.trim();
        if (asinInput) {
            // Redirect to second page with the ASIN parameter
            window.location.href = "second.html?url=" + encodeURIComponent(asinInput);
        }
    });
});
